...
import { enforce } from '../rbac/enforce.js';
...
// Pipelines
crmRouter.get('/api/v1/crm/pipelines', enforce('read'), async (req, res) => {
  const items = await repo.list('pipelines', tenant(req), parseInt(String(req.query.limit||'50')));
  res.json({ items });
});

crmRouter.post('/api/v1/crm/pipelines', enforce('write'), async (req, res) => {
...
crmRouter.get('/api/v1/crm/pipelines/:id', enforce('read'), async (req, res) => {
  const r = await repo.get('pipelines', tenant(req), req.params.id);
  if (!r) return res.status(404).json({ error: 'not found' });
  res.json(r);
});

crmRouter.patch('/api/v1/crm/pipelines/:id', enforce('write'), async (req, res) => {
...
crmRouter.delete('/api/v1/crm/pipelines/:id', enforce('admin'), async (req, res) => {
...
// Contacts
crmRouter.get('/api/v1/crm/contacts', enforce('read'), async (req, res) => {
  const items = await repo.list('contacts', tenant(req), parseInt(String(req.query.limit||'50')));
  res.json({ items });
});
crmRouter.post('/api/v1/crm/contacts', enforce('write'), async (req, res) => {
...
crmRouter.get('/api/v1/crm/contacts/:id', enforce('read'), async (req, res) => {
  const r = await repo.get('contacts', tenant(req), req.params.id);
  if (!r) return res.status(404).json({ error: 'not found' });
  res.json(r);
});
crmRouter.patch('/api/v1/crm/contacts/:id', enforce('write'), async (req, res) => {
...
crmRouter.delete('/api/v1/crm/contacts/:id', enforce('admin'), async (req, res) => {
...
// Opportunities
crmRouter.get('/api/v1/crm/opportunities', enforce('read'), async (req, res) => {
  const items = await repo.list('opportunities', tenant(req), parseInt(String(req.query.limit||'50')));
  res.json({ items });
});
crmRouter.post('/api/v1/crm/opportunities', enforce('write'), async (req, res) => {
...
crmRouter.get('/api/v1/crm/opportunities/:id', enforce('read'), async (req, res) => {
  const r = await repo.get('opportunities', tenant(req), req.params.id);
  if (!r) return res.status(404).json({ error: 'not found' });
  res.json(r);
});
crmRouter.patch('/api/v1/crm/opportunities/:id', enforce('write'), async (req, res) => {
...
crmRouter.delete('/api/v1/crm/opportunities/:id', enforce('admin'), async (req, res) => {
...
// Import (CSV/JSON)
crmRouter.post('/api/v1/crm/import', enforce('write'), upload.single('file'), async (req, res) => {
...
// Export
crmRouter.get('/api/v1/crm/export', enforce('read'), async (req, res) => {
  const tenantId = tenant(req);
