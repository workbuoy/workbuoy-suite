...
function expandCandidate(candidate: string): string[] {
  ...
  const roots = [process.cwd(), REPO_ROOT];
  const expanded = roots.map((root) => path.resolve(root, candidate));
  return Array.from(new Set(expanded.map((item) => path.normalize(item))));
}

function loadJSON(abs: string): JsonCandidate {
...
function tryLoadList(candidatePaths: string[], key: 'roles' | 'features'): LoadResult<any> | null {
  for (const candidate of candidatePaths) {
    for (const abs of expandCandidate(candidate)) {
  ...
  try {
    ...
    if (loaded) {
      if (envOverride && envOverride.trim()) {
        const envCandidates = expandCandidate(envOverride.trim()).map((p) => path.resolve(p));
        if (envCandidates.includes(path.resolve(loaded.path))) {
