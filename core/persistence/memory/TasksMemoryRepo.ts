...
export class TasksMemoryRepo implements Repository<Task> {
  private items: Task[] = [];
  async create(data: Task) { this.items.push(data); return data; }
  ...
  async find({limit=50, offset=0, q}: any = {}) {
    return this.items.filter(c => !q || c.title.includes(q))
