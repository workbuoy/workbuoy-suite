...
(async () => {
  ...
  const post1 = db.getCache<{ name: string }>('c9')!.payload;
  ...
  await engine.syncOnce();
  if (db.getCache<{ name: string }>('c9')!.payload.name !== 'Local Newer') { console.error('LWW local not chosen'); process.exit(1); }
  ...
  const merged = db.getCache<{ name: string }>('c9')!.payload.name;
