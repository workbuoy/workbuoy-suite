...
import { integrationsConnectStarted, integrationsConnected, integrationsFailed, wbIntegrationConnect } from '../../../lib/metrics/integrations.js';
...
function /* patched for metrics */ setStatus(user_id, provider, status, meta){
  withDb(db=>{
...
export default async function handler(req,res){
  ...
  try{
    ...
    integrationsConnectStarted.labels(provider).inc(); wbIntegrationConnect.labels(provider,'started').inc();
    /* patched for metrics */ setStatus(user_id, provider, 'pending', { step:'oauth_start' });
    if(process.env.NODE_ENV !== 'production'){
      /* patched for metrics */ setStatus(user_id, provider, 'connected', { demo:true });
...
// NOTE: wbIntegrationConnect increments added in success/failure paths where applicable.
