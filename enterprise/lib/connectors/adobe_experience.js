...
async function sync(tenant){
  const name = 'Adobe Experience';
  const labels = { connector: name, tenant };
  metrics.counters.connector_sync_total.inc(labels);
  const sinceKey = 'since';
  const maxPerPage = 200;
  try {
    const secret = await getSecret(process.env['ADOBE_EXPERIENCE_SECRET'] || `${tenant}:ADOBE_EXPERIENCE_SECRET`);
    ...
    while (page <= 1){ // extend when real API paging is wired
      const url = 'https://analytics.adobe.io/api/{tenant}/reports'.replace('{tenant}', tenant);
      ...
      const inc = filterIncremental(arr, since, 'lastModified');
      items = items.concat(inc);
      break;
    }
    await appendAudit({ type:'connector_sync', connector: name, tenant, count: items.length });
    const maxTs = items.reduce((m,it)=>{ const v = it['lastModified']||it['updated_at']||it['lastModifiedDateTime']; const t = typeof v==='number'? v : Date.parse(v); return isNaN(t)? m : Math.max(m,t); }, since||0);
    ...
    await appendAudit({ type:'connector_error', connector:'Adobe Experience', tenant, error: String(e.message||e) });
    return { ok:false, error:String(e.message||e) };
  }
}

module.exports = { name: 'Adobe Experience', sync };
