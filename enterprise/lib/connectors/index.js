import { getRefKeyFor } from '../../lib/config/tenant-secrets.js';
...
export async function runConnector(name, since, tenantId) {
  ...
  return await withIncrementalSync(name, 'default', since, async (effectiveSince) => {
    const keys=[]; for(const k of cfg.keys){ const ref= tenantId? await getRefKeyFor(tenantId,k):null; keys.push(ref||k);} const result = await chooseFetch(name)(name, keys);
    return result;
  });
}
