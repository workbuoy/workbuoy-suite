...
async function sync(tenant){
  ...
  try {
    const secret = await getSecret(process.env['WORKDAY_SECRET'] || `${tenant}:WORKDAY_SECRET`);
    ...
    while (page <= 1){ // extend when real API paging is wired
      const url = 'https://wd2-impl-services1.workday.com/ccx/api/v1/{tenant}/workers'.replace('{tenant}', tenant);
    ...
    await appendAudit({ type:'connector_error', connector:'Workday', tenant, error: String(e.message||e) });
    return { ok:false, error:String(e.message||e) };
  }
}

module.exports = { name: 'Workday', sync };
