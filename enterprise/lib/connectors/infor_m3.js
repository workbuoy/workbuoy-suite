...
async function sync_infor_m3(tenant){
  ...
  try {
    ...
    const itemsInc = filterIncremental(rawItems, since, 'lastUpdateDate');
    for (const it of itemsInc) {
      items.push(it);
    }
    await appendAudit({ type: 'connector_sync', connector: name, tenant, count: items.length });
    const maxTs = itemsInc.reduce((m,it)=>{ const v = it['lastUpdateDate']||it['updated_at']||it['lastModifiedDateTime']; const t = typeof v==='number'? v : Date.parse(v); return isNaN(t)? m : Math.max(m,t); }, since||0);
    ...
    await appendAudit({ type: 'connector_error', connector: 'Infor M3', tenant, error: String(e.message||e) });
    return { ok: false, error: String(e.message||e) };
  }
}

module.exports = { name: 'Infor M3', sync: sync_infor_m3 };
