...
export class DLQ {
  constructor({ redisUrl, filePath }={}){
    ...
    this.filePath = filePath || path.join(process.cwd(),'connectors','salesforce','dlq.json');
    if (this.redisUrl){
  ...
  async push(item){
    if (this.redis){
      try{
        ...
        await this.redis.lpush('wb:sf:dlq', JSON.stringify(item));
        return;
      }catch(e){ /* fall through to file */ }
    }
    // file fallback
    let arr = [];
    try { arr = JSON.parse(fs.readFileSync(this.filePath,'utf8')); } catch {}
    arr.push(item);
  ...
  async depth(){
    if (this.redis){
      try{ if (!this.redis.status || this.redis.status === 'end') await this.redis.connect(); return await this.redis.llen('wb:sf:dlq'); }catch{}
    }
