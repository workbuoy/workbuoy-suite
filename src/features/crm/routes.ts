...
// In-memory store (delta-safe: can be swapped for prisma repo later)
const items: any[] = [];

// Policy guard (strict)
import { policyV2Guard } from "../../core/policyV2/middleware";
// Event bus & audit fallbacks (delta-safe)
...
const logger = tryReq<any>("../../core/logging/logger") || { info: console.log };
...
router.post("/api/crm/contacts", policyV2Guard("write","low"), (req, res, next) => {
  try {
    ...
    logger?.info?.({ tag: "crm.create", id: item.id });
    res.status(201).json({ item, correlationId: (req as any).wb?.correlationId, explanations: [] });
  } catch (e) { next(e); }
});

router.delete("/api/crm/contacts/:id", policyV2Guard("write","low"), (req, res, next) => {
  try {
